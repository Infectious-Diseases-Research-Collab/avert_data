-- Adds two duplicate checks to refresh_quality_issues().
--
--   20. duplicate_barcode  (error)   -- the clinic/lab join key on two records
--   21. duplicate_subjid   (warning) -- one subject ID issued to two children
--
-- Generated from the deployed definition, with only these two checks added;
-- checks 1-19 are unchanged. Run in the Supabase SQL editor, then re-dump
-- supabase/avert_dashboard.json.
...
begin
  ...
  select e.country, 'duplicate_barcode', 'error',
         min(e.subjid), e.barcode, min(e.mrc), 'barcode', null,
  ...
  select e.country, 'duplicate_subjid', 'warning',
         e.subjid, min(e.barcode), min(e.mrc), 'subjid', null,
